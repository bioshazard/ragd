...
- Ingest chunking (default): target ~400–800 tokens per chunk (600 tokens typical) with ~10–20% overlap (80–150 tokens), split on segment boundaries/topic shifts, then sentences/paragraphs, then token-based fallback; allow overrides for chunk size/overlap/limits; use ~4k char upper bound (~1k tokens) unless explicitly configured otherwise.
- Direction: two-layer system — core RAG decomposition (basic retrieval blocks, stop before agent/LLM pipelines) + expose as a library and as REST/MCP service (library is source of truth, service is a thin adapter).
- Core goals: minimal deps, well-specified behavior, conformance/golden tests, composable blocks with stable interfaces, reproducible ingest/eval (deterministic ingest + harness).
...
- DB: Postgres-first core schema encoding contracts (chunks, provenance, embeddings, metadata filters) with compatibility/versioning story.
